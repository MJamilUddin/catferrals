...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // For GET requests, return verification status
  ...
  try {
    ...
    // Check if already verified (token cleared means verified)
    if (referral.referrerAccount.isEmailVerified && !referral.referrerAccount.emailVerificationToken) {
      return json({ 
        success: true, 
        verified: true,
        referrer: {
    ...
    // Check if verification token matches for unverified accounts
    ...
    return json({ 
      success: true, 
      verified: referral.referrerAccount.isEmailVerified,
      referrer: {
        email: referral.referrerAccount.email,
        name: `${referral.referrerAccount.firstName || ''} ${referral.referrerAccount.lastName || ''}`.trim() || 'Referrer'
      }
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    const body = await request.json() as { code: string; verify: string; shop: string };
...
// Handle OPTIONS requests as well
